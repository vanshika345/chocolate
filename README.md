...
To check whether your approach is correct for challenges given below --> you can go to `tests/FunctionsTest.js` file, and uncomment the section from line number 764-828.
This will basically unlock the test cases for the challenges given below.

### Challenge 1: Fresh pickings!

Cait believes that all the best and newest chocolates are at the top of the machine. She needs help removing just one chocolate of the given color from the top. Create a function `removeChocolateOfColor()` which takes an argument `chocolates,color` and removes one chocolate of the given color from the top.
